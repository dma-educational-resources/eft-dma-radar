namespace eft_dma_radar.Arena.Misc.Pools
{
        ...
        where T : class, IPooledObject<T>
    {
        ...
        {
            if (obj is IPooledObject<T> p) { p.SetDefault(); ObjectPool.Return(obj); }
            else Log.WriteLine($"CRITICAL: Cannot return '{obj.GetType()}' to pool.");
        }
        ...
        private static class ObjectPool
        {
            ...
            internal static T Rent() => _pool.TryPop(out var obj) ? obj : _factory();
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal static void Return(T obj) { if (_pool.Count < MaxPoolSize) _pool.Push(obj); }
        }
    }
}
