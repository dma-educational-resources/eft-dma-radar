        ...
        where TValue : unmanaged
    {
        ...
        public static UnityDictionary<TKey, TValue> Create(ulong addr, bool useCache = true)
        {
            ...
            try
            {
                ...
                var dictBase = Tarkov.MemoryInterface.Memory.ReadPtr(addr + EntriesOffset, useCache) + EntriesStartOffset;
                Tarkov.MemoryInterface.Memory.ReadBuffer(dictBase, dict.Span, useCache); // Single read into mem buffer
                return dict;
            }
            catch
            {
